...
// BinaryNode.DeleteNode deletes the given node in the given treeRoot and returns it.
...
func (treeRoot *BinaryNode) DeleteNode(node *BinaryNode) (*BinaryNode, error){
	if treeRoot == nil{
		return nil, errors.New("Given treeRoot is nil.")
	} else if node == nil{
		return nil, errors.New("Trying to delete node that doesn't exit.")
	}
...
func (treeRoot *BinaryNode) Delete(key int) (int, error){
	...
	if nodeToDelete == nil{
		return 0, errors.New("Given key doesn't associated to any node in this tree.")
	}

	deletedNode, err := treeRoot.DeleteNode(nodeToDelete)
	if err != nil{
		return 0, err
	}
