...
typedef struct node 
{
	...
	double weight;		// weight of the given node from some root.
...
// create_node create a new node with the given value and the weight.
// return NULL if there's not enough memory for creating node.
node* create_node(char value, int weight);
...
// add_edge adds a new edge to the given graph with the given source and destination with the given weight.
...
void add_edge(graph *graph, char source, char destination, int weight);

// add_edges adds new edges with the given graph with the source and the given destination values.
void add_edges(graph *graph, char source, char values[], int weights[], int length);
