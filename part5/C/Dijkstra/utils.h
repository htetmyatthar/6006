...
typedef struct 
{
	queue_node* queue_vertix;		// queue vertix array
	queue_node** queue_map;			// map to vertix array for changing distance (key)
	int size;
}changable_PQ_array;
...
typedef struct
{
	queue_node* queue_vertex;	// queue vertex array
	queue_node** queue_map;		// map to vertex array for changin distace(priority key)
	int size;
